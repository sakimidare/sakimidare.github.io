...
开幕雷击，压了一百万个栈。在栈中分配了 `0x50` 字节空间后把栈指针赋给了 `%r13`。然后又调用了 `read_six_numbers` 往内存里面存数字，内存布局还是见[上一张表](#target-point)。存完数字之后把栈指针 `%rsp` 存到 `%r14` 里面去。
